            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div class="mt-2">
                    <p class="text-sm text-muted font-mono">
                        ...
                        <button form="send-verification" class="text-label hover:text-text underline font-mono text-sm transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-main rounded">
                            {{ __('Click here to re-send the verification email.') }}
                        </button>
                    </p>

                    @if (session('status') === 'verification-link-sent')
                        <p class="mt-2 text-sm text-live font-mono">
        ...
        <div class="flex items-center gap-4 pt-2">
            ...
            @if (session('status') === 'profile-updated')
                <p
                    ...
                    class="text-sm text-live font-mono"
                >{{ __('Saved.') }}</p>
            @endif
        </div>
    </form>
